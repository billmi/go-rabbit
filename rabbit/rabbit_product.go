...
func (p *RabbitProduct) GetType(tTag string) string {
	...
	switch tTag {
	case "H":
		_t = amqp.ExchangeHeaders  //  routingkey will be not uesd   header模式与routing不同的地方在于，header模式取消routingkey，使用header中的 key/value（键值对）匹配队列。
		break
	case "F":
		_t = amqp.ExchangeFanout   // p / s
		break
	case "T":
		_t = amqp.ExchangeTopic    //   example  =>  *.critical   [wide than fanout]
		break
	case "D":
		_t = amqp.ExchangeDirect   //   direct
		break
	default:
		_t = amqp.ExchangeFanout	// p / s
	}
...
func (p *RabbitProduct) QueueSimple(queue string,  msg string, durable bool, autoDelete bool) (bool, error) {
	...
	q, err := p.Channel.QueueDeclare(
		...
		false,      // exclusive
		false,      // no-wait
		nil,        // arguments   if you need
	)
	if err != nil {
		return false, err
	}
	if err := p.UnsafePush("", q.Name, msg); err != nil {
		return false, err
	}
	return true, nil
}
